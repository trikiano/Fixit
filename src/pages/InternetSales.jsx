...
import { Wifi, Plus, Search, FileText, TrendingUp, Users, DollarSign, Trash2, Filter } from 'lucide-react';
...
// Comptes/applis configurés (modifiable ici)
...
export default function InternetSalesPage() {
  ...
  // Grouper par compte pour le résumé
  const byAccount = sales.reduce((acc, s) => {
  ...
  return (
    <div>
      <PageHeader title="Forfaits Internet" subtitle="Ventes de forfaits et historique par compte">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-xl font-bold">{filtered.length}</p><p className="text-xs text-muted-foreground">Ventes filtrées</p></div>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-xl font-bold text-green-400">{totalRevenue.toFixed(2)} €</p><p className="text-xs text-muted-foreground">CA filtré</p></div>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            <div className="h-9 w-9 rounded-lg bg-blue-500/10 flex items-center justify-center"><TrendingUp className="h-4 w-4 text-blue-400" /></div>
            <div><p className="text-xl font-bold text-blue-400">{totalProfit.toFixed(2)} €</p><p className="text-xs text-muted-foreground">Bénéfice net</p></div>
          </CardContent>
        </Card>
        <Card className="border-border/50">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div><p className="text-xl font-bold">{new Set(sales.map(s => s.client_phone)).size}</p><p className="text-xs text-muted-foreground">Clients uniques</p></div>
          </CardContent>
        </Card>
      </div>
      ...
      {Object.keys(byAccount).length > 0 && (
        <div className="mb-6">
          <p className="text-sm font-semibold mb-3 text-muted-foreground uppercase tracking-wider">Résumé par compte</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {Object.entries(byAccount).map(([account, data]) => (
              <Card key={account} className="border-border/50 cursor-pointer hover:border-primary/50 transition-colors" onClick={() => openReport(account)}>
                <CardContent className="p-4 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-green-400">{data.revenue.toFixed(2)} €</p>
                    <Button variant="ghost" size="sm" className="text-xs h-6 px-2 mt-1" onClick={(e) => { e.stopPropagation(); openReport(account); }}>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/20">
                    ...
                    <th className="text-left p-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Statut</th>
                    <th className="p-3"></th>
                  </tr>
                </thead>
                <tbody>
